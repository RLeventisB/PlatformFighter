    ...
    public abstract class BaseVertexBatch<T> : IDisposable where T : struct, IVertexType
    {
        ...
        public virtual unsafe void DrawTrail()
        {
            if (vertices.Count >= 3)
            {
                ...
                *ptr = num * Camera.matrix.M11 - Camera.matrix.M14;
                ptr[1] = num * Camera.matrix.M21 - Camera.matrix.M24;
                ptr[2] = num * Camera.matrix.M31 - Camera.matrix.M34;
                ptr[3] = num * Camera.matrix.M41 - Camera.matrix.M44;
                ptr[4] = num2 * Camera.matrix.M12 + Camera.matrix.M14;
                ptr[5] = num2 * Camera.matrix.M22 + Camera.matrix.M24;
                ptr[6] = num2 * Camera.matrix.M32 + Camera.matrix.M34;
                ptr[7] = num2 * Camera.matrix.M42 + Camera.matrix.M44;
                ptr[8] = Camera.matrix.M13;
                ptr[9] = Camera.matrix.M23;
                ptr[10] = Camera.matrix.M33;
                ptr[11] = Camera.matrix.M43;
                ptr[12] = Camera.matrix.M14;
                ptr[13] = Camera.matrix.M24;
                ptr[14] = Camera.matrix.M34;
                ptr[15] = Camera.matrix.M44;
